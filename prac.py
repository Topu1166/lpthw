...
# 
# #using input method
# file_Name = input("Enter the file name to read its content: ") 
# 
# target1 = open(file_Name, 'r+') 
# print(target1.read())
# 
# print("We are goint to remove all the contents of the file." ) 
# print("If you don't want it, press ctrl + c to abort: ") 
# print("Or Enter to continue...") 
# 
# input('?') 
# print('---' * 10, "\nWe are truncating the file.....\n") 
# 
# target1.seek(0) 
# target1.truncate() 
# print("I am going to ask you for three lines.")
...
def converter(blocks):
    KB = blocks 
    MB = blocks / 1024
    GB = MB / 1024
    return KB, MB, GB 

byte = int(input("Ener the number of blocks/kilobytes: ")) 
KB, MB, GB = converter(byte) 
print(f"Kilobytes: {KB}\nMegabytes: {round(MB, 2)}\nGigabytes: {round(GB, 2)}\n") 

#Find the vowels from a sentence/string. 

def all_vowels(sentence): 
    vowels = "AEIOUaeiou" 
    count = 0 
    vowels_basket = []  
    for char in sentence:
        if char in vowels: 
            count += 1 
            vowels_basket.append(char.lower()) 
    return count, vowels_basket 

sen = input("Enter a sentence to see how many vowels there are:\n>> ")
count, vowels = all_vowels(sen) 
print(f"There are {count} vowels in the sentence.\n{vowels} ")

#Write a for loop that prints the integers from the following/a list, but onlyif they are even. 

numbers = [1, 3, 8, 4, 9, 3, 88, 25] 
